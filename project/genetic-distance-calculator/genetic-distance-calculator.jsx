...
import {
  parseMapTsv,
  calculateSegmentCm,
  resolveChromosomeKey,
  normalizeChromosomeText,
  toNumber,
  toOptionalNumber,
} from "./map-utils.js";
import { parseTabularFile } from "./file-handler.js";
import { styles } from "./styles/genetic-distance-calculator-styles.jsx";

const CM_DELTA_ALERT_THRESHOLD = 0.1;
const MAT_PAT_IMBALANCE_THRESHOLD = 0.15;

function formatCellDecimal(value, decimals = 1) {
...
export default function GeneticDistanceCalculatorApp() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const [matText, patText] = await Promise.all([
          loadText(matMapUrl),
          loadText(patMapUrl),
        ]);
        if (!active) return;
  ...
  useEffect(() => {
    const normalizedHeaders = tableHeaders.map((header) =>
    ...
    const pickAdjacent = (anchorIndex, offset) => {
      ...
      return targetIndex >= 0 && targetIndex < tableHeaders.length
        ? tableHeaders[targetIndex]
        : "";
    };

    const chrIndex = normalizedHeaders.findIndex(
      (header) =>
        /^chr$/.test(header) ||
        /^chrom$/.test(header) ||
        /^chromosome/.test(header),
    );
    ...
    const inferredStart =
      pick([/^start$/, /start[_\s-]?pos/, /^from$/]) ||
      (chrIndex >= 0 ? pickAdjacent(chrIndex, 1) : "");
    const inferredEnd =
      pick([/^end$/, /end[_\s-]?pos/, /^to$/]) ||
      (chrIndex >= 0 ? pickAdjacent(chrIndex, 2) : "");
  ...
  function onCalculate(event) {
    ...
    if (startBp === null || endBp === null || startBp < 0 || endBp < 0) {
      setFormError(
        "Start and end must be valid non-negative base-pair positions.",
      );
      return;
    }
    ...
    const maternalCm = matIntervals.length
      ? calculateSegmentCm(matIntervals, startBp, endBp)
      : null;
    const paternalCm = patIntervals.length
      ? calculateSegmentCm(patIntervals, startBp, endBp)
      : null;
    const averagedCm =
      maternalCm !== null && paternalCm !== null
        ? (maternalCm + paternalCm) / 2
        : (maternalCm ?? paternalCm);
  ...
  async function onFileSelected(file) {
    ...
    try {
      ...
      if (!parsed.headers.length || !parsed.rows.length) {
        setTableError(
          "The uploaded file did not contain tabular rows with a header.",
        );
        return;
      }
  ...
  const tableComputedRows = useMemo(() => {
    ...
    return tableRows.map((row) => {
      ...
      if (startBp !== null && endBp !== null && startBp >= 0 && endBp >= 0) {
        ...
        maternalCm = matIntervals.length
          ? calculateSegmentCm(matIntervals, startBp, endBp)
          : null;
        paternalCm = patIntervals.length
          ? calculateSegmentCm(patIntervals, startBp, endBp)
          : null;
        averagedCm =
          maternalCm !== null && paternalCm !== null
      ...
      return {
        ...
        existingCm: formatCellDecimal(existingRaw, 1),
        maternalCm,
        paternalCm,
        averagedCm,
      };
  ...
  useEffect(() => {
    ...
    const stillExists = tableChromosomeOptions.some(
      (option) => normalizeChromosomeText(option) === selected,
    );
    if (!stillExists) {
  ...
  const filteredSortedRows = useMemo(() => {
    ...
    const nameFilter = String(tableFilters.name || "")
      .trim()
      .toLowerCase();
    ...
    const filtered = tableComputedRows.filter((row) => {
      ...
      if (nameFilter) {
        ...
        if (!nameText.includes(nameFilter)) return false;
      }

      if (chrFilter && chrText !== chrFilter) return false;
      if (startMinBp !== null && (startBp === null || startBp < startMinBp))
        return false;
      if (endMaxBp !== null && (endBp === null || endBp > endMaxBp))
        return false;
    ...
    const sorted = [...filtered].sort((a, b) => {
      if (tableSort.key === "chr") {
        ...
        return (
          String(a.chr || "").localeCompare(String(b.chr || "")) * dirFactor
        );
      }
  ...
  const totalTablePages = Math.max(
    1,
    Math.ceil(totalTableRows / tablePageSize),
  );
  ...
  return (
    <section style={styles.card}>
      ...
      <p style={{ marginTop: 0, marginBottom: 14, color: "#4f5f73" }}>
        Calculate the genetic distance in centimorgans (cM) across DNA segments
        using recombination maps. Enter a chromosome and start/end positions
        manually, or drop a table file (for example, '23andMe relatives
        download') to map columns and calculate cM for multiple segments at
        once.
      </p>
      <div
      ...
      >
        <div style={{ fontSize: 13, color: "#4f5f73" }}>
          Drop CSV, TXT, TSV, XLSX, or XLS here to switch to table mode and map
          columns.
          {tableFileName ? (
            <strong style={{ marginLeft: 8 }}>Loaded: {tableFileName}</strong>
          ) : null}
        </div>
        <div
          style={{
      ...
      {tableError ? (
        <div style={{ color: "#b10020", marginBottom: 10 }}>{tableError}</div>
      ) : null}

      <div style={mode === "table" ? { display: "block" } : styles.pageGrid}>
        <div>
          {loading ? <div>Loading recombination maps...</div> : null}
          {loadError ? (
            <div style={{ color: "#b10020" }}>{loadError}</div>
          ) : null}
          ...
          {!loading && !loadError && mode === "manual" ? (
            <form onSubmit={onCalculate}>
              ...
              >
                ...
                {formError ? (
                  <span style={{ color: "#b10020" }}>{formError}</span>
                ) : null}
              </div>
            </form>
          ) : null}
              ...
              >
                ...
                <div style={styles.grid}>
                  ...
                  <label>
                    ...
                    <select
                      value={columnMap.existingCm}
                      onChange={(e) =>
                        setColumnMap((prev) => ({
                          ...prev,
                          existingCm: e.target.value,
                        }))
                      }
                      style={{ display: "block", width: "100%", marginTop: 6 }}
                    >
                ...
                >
                      ...
                      >
                        ...
                        >
                          ...
                          {tableChromosomeOptions.map((chrOption) => (
                            <option key={chrOption} value={chrOption}>
                              {chrOption}
                            </option>
                          ))}
                        </select>
                  ...
                  <tbody>
                    {pagedTableRows.map((row, idx) => {
                      const existingNumeric = toOptionalNumber(
                        String(row.existingCm || "").replace(/,/g, ""),
                      );
                      const averageDelta =
                      ...
                      const matPatDeltaFromAveragePct =
                        ...
                        row.averagedCm !== null &&
                        row.averagedCm !== 0
                          ? Math.abs(row.maternalCm - row.averagedCm) /
                            row.averagedCm
                          : null;
                      const showMatPatMarker = Boolean(
                        matPatDeltaFromAveragePct !== null &&
                        matPatDeltaFromAveragePct > MAT_PAT_IMBALANCE_THRESHOLD,
                      );
                      const matPatMarker = !showMatPatMarker
                        ? ""
          ...
          {mode === "manual" && result ? (
            ...
            >
              ...
              >
                <div>
                  {result.chromosome} | Segment: {result.start.toLocaleString()}{" "}
                  - {result.end.toLocaleString()} bp
                </div>
                <div
                  style={{
                    width: 1,
                    alignSelf: "stretch",
                    background: "rgba(106, 118, 134, 0.35)",
                  }}
                />
                <div>
          ...
          <aside style={styles.infoPanel}>
            ...
            <div style={{ color: "#4f5f73", lineHeight: 1.55 }}>
              <p style={{ margin: "0 0 10px" }}>
                Calculate centimorgans (cM) between the start and end of a DNA
                segment using the updated recombination-rate values published by
                the DECODE project.
              </p>
              <p style={{ margin: "0 0 10px" }}>
                These values come from a recent paper (Palsson et al., 2025) and
                its accompanying repository, which provide newer maternal and
                paternal recombination maps than the older reference tables used
                by many calculators. The updated maps incorporate both
                non-crossover (NCO) and crossover (CO) information when deriving
                cM/Mb rates.
              </p>
              <p style={{ margin: "0 0 10px" }}>
                Palsson, G., Hardarson, M. T., Jonsson, H., Steinthorsdottir,
                V., Stefansson, O. A., Eggertsson, H. P., ... &amp; Stefansson,
                K. (2025). Complete human recombination maps. Nature, 1-8.{" "}
                <a
                  href="https://doi.org/10.1038/s41586-024-08450-5"
                  target="_blank"
                  rel="noreferrer"
                >
              ...
              <p style={{ margin: "0 0 12px" }}>
                Data source:{" "}
                <a
                  href="https://github.com/DecodeGenetics/PalssonEtAl_Nature_2024"
                  target="_blank"
                  rel="noreferrer"
                >
                  deCODE GitHub Repository
                </a>
                .
              </p>
            </div>
            ...
            >
              ...
              <div style={{ fontSize: 13, color: "#4f5f73", lineHeight: 1.5 }}>
                Planned: drop CSV, TXT, or Excel files, map chromosome/start/end
                columns (and optional existing cM), then append maternal,
                paternal, and averaged cM columns.
              </div>
            </div>
          </aside>
        ) : null}
